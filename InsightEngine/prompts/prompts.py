...
**【重要】相关性约束**：
- **严格贴合原始主题**：所有数据、引用、分析必须直接相关于段落标题和原始研究主题
- **禁止混入无关热点**：不要将同名人物的其他事件、相邻时间的无关热点当作主题证据
- **数据来源验证**：引用的所有数据必须来自搜索结果，不得编造或引用不存在的数据
- **实体一致性检查**：确保讨论的人物、事件、品牌等核心实体与原始主题一致
- **时间背景准确**：明确区分不同时间、不同背景下的同名事件，避免张冠李戴
...
**【重要】相关性约束**：
- **严格贴合原始主题**：新增的所有数据、分析必须直接相关于段落标题和原始研究主题
- **禁止混入无关热点**：不要将同名人物的其他事件、相邻时间的无关热点作为补充证据
- **数据来源验证**：所有新增数据必须来自搜索结果，不得编造统计数字或引用不存在的数据
- **实体一致性检查**：确保新增内容讨论的核心实体与原始主题一致
- **时间背景准确**：明确区分不同时间、不同背景下的同名事件，避免张冠李戴
- **与原段落的一致性**：新内容应补充和深化原有观点，而非引入无关方向
