"""
LangGraph状态定义 (MediaEngine) - 使用TypedDict和Reducer模式
支持checkpoint和状态回溯

与 QueryEngine/langgraph_state.py 结构一致, 仅状态类名不同
(MediaGraphState)。段落字典结构、reducer 语义、辅助函数完全相同,
以便三个 DeepSearch 引擎共享同一套图执行约定 (各自保留独立副本)。
"""
...
def create_initial_state(query: str, max_reflections: int = 2, max_paragraphs: int = 5) -> MediaGraphState:
    """
    创建初始状态
...
def add_paragraph(state: MediaGraphState, title: str, content: str) -> Dict:
    """添加段落到状态 (返回更新字典)"""
    new_paragraph = {
        "title": title,
        "content": content,
...
def update_paragraph_summary(state: MediaGraphState, paragraph_index: int, summary: str) -> Dict:
    """更新段落总结"""
...
def increment_reflection(state: MediaGraphState, paragraph_index: int) -> Dict:
    """增加反思计数"""
    paragraphs = state["paragraphs"].copy()
    if 0 <= paragraph_index < len(paragraphs):
        paragraph = paragraphs[paragraph_index].copy()
...
def mark_paragraph_completed(state: MediaGraphState, paragraph_index: int) -> Dict:
    """标记段落完成"""
    paragraphs = state["paragraphs"].copy()
    if 0 <= paragraph_index < len(paragraphs):
        paragraph = paragraphs[paragraph_index].copy()
...
def add_message(state: MediaGraphState, message: str) -> Dict:
    """添加消息到历史"""
    return {
...
def add_error(state: MediaGraphState, error: str) -> Dict:
    """添加错误到列表"""
    return {
