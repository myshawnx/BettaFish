...
为 Insight Agent 提供友好的 Web 界面 —— 作为 app.py 主控制台的 insight 子应用被 iframe 嵌入。

与原版 insight_engine_streamlit_app.py 保持相同的"嵌入契约":
- 从 URL 读取 ?query=...&auto_search=true, 由主页搜索框驱动
- 配置硬编码, 从全局 settings 读取 (无侧边栏手动输入)
- 只读查询展示 + error_with_issue_link 错误处理

区别仅在引擎: 使用 LangGraphInsightAgent (StateGraph + SqliteSaver checkpoint),
而非旧的 DeepSearchAgent。流程: 结构 -> 逐段搜索/总结 -> 反思循环 -> 推进段落 -> 最终报告。
"""
...
RECOVERY_STATE_KEY = "insight_langgraph_api_recovery"
...
def build_insight_config(values: dict | None = None) -> Settings:
    """Build an InsightEngine Settings object from submitted values plus current settings."""
    return Settings(
        INSIGHT_ENGINE_API_KEY=_setting_value(values, "INSIGHT_ENGINE_API_KEY"),
        INSIGHT_ENGINE_BASE_URL=_setting_value(values, "INSIGHT_ENGINE_BASE_URL"),
        INSIGHT_ENGINE_MODEL_NAME=_setting_value(values, "INSIGHT_ENGINE_MODEL_NAME") or "qwen-max",
    ...
    return [
        {
            ...
            "label": "Insight LLM API Key",
            "value": _setting_value(values, "INSIGHT_ENGINE_API_KEY"),
            "secret": True,
            "required": True,
        },
        {
            "key": "INSIGHT_ENGINE_BASE_URL",
            "label": "Insight LLM Base URL",
            "value": _setting_value(values, "INSIGHT_ENGINE_BASE_URL"),
        },
        {
            "key": "INSIGHT_ENGINE_MODEL_NAME",
            "label": "Insight LLM Model",
            "value": _setting_value(values, "INSIGHT_ENGINE_MODEL_NAME") or "qwen-max",
            "required": True,
        },
    ]
...
def main():
    """主函数"""
    st.set_page_config(
        page_title="Insight Agent (LangGraph)",
        page_icon="",
        layout="wide"
    )
    ...
    if start_research:
        ...
        if not settings.INSIGHT_ENGINE_API_KEY:
            message = "缺少必要 API 配置: INSIGHT_ENGINE_API_KEY"
            st.error(message)
            logger.error(message)
            _remember_api_recovery(query, None, message, mode="start")
        else:
            execute_research(query, build_insight_config())
...
def render_pending_api_recovery():
    ...
    merged_values = render_api_recovery_form(
        form_key="insight_api_recovery_form",
        engine_label="Insight Agent",
    ...
    thread_id = recovery.get("thread_id")
    config = build_insight_config(merged_values)
    _clear_api_recovery()
...
def _run_research(query: str, config: Settings, thread_id: str | None, resume: bool) -> bool:
    ...
    try:
        ...
        agent = LangGraphInsightAgent(config=config, checkpoint_dir=".checkpoints")
        ...
        error_display = error_with_issue_link(
            f"研究过程中发生错误: {str(e)}",
            error_traceback,
            app_name="Insight Engine LangGraph Streamlit App"
        )
...
def display_results(final_state: dict, final_report: str):
    ...
    with tab2:
        ...
        if all_searches:
            ...
            st.info("本次研究未命中数据库结果 (数据库为空时属正常)。")
