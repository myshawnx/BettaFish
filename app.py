...
STREAMLIT_SCRIPTS = {
    # LangGraph 版 insight 子应用 (StateGraph + checkpoint)。
    # 原版旧 agent 实现保留在 insight_engine_streamlit_app.py 作为回退。
    'insight': 'SingleEngineApp/insight_engine_langgraph_streamlit_app.py',
    # LangGraph 版 media 子应用 (StateGraph + checkpoint, Bocha/Anspire 多模态搜索, 后端由 SEARCH_TOOL_TYPE 决定)。
    # 原版旧 agent 实现保留在 media_engine_streamlit_app.py 作为回退。
    'media': 'SingleEngineApp/media_engine_langgraph_streamlit_app.py',
    # LangGraph 版 query 子应用 (StateGraph + checkpoint, Tavily 网络搜索)。
    # 原版旧 agent 实现保留在 query_engine_streamlit_app.py 作为回退。
    'query': 'SingleEngineApp/query_engine_langgraph_streamlit_app.py'
}
